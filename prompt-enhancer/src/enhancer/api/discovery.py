...
The four-product loop (Prompt Enhancer ↔ Round Robin ↔ Interpreter ↔
Loop Driver) discovers each other through a shared TOML file at:
...
loopback ports so dev still works without the file present. Modular
constraint: no in-process shared state, no cross-product imports —
just a config file the user controls.
"""
