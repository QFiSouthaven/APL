...
Mirror of ``enhancer.api.discovery`` and ``round_robin.discovery`` —
same lookup order, same ``DEFAULTS`` dict (byte-for-byte), same
swallow-everything error policy. The four products
(prompt-enhancer / round-robin / development / hardware-info) all read
the same shared TOML file at:

* Windows: ``%APPDATA%\\swarm\\services.toml``
* Linux/macOS: ``~/.config/swarm/services.toml``

so they agree on each other's URLs without imports between packages.
Each product reads on demand (no caching). Defaults to localhost
loopback ports so dev still works without the file present.
"""
...
# IDENTICAL to enhancer.api.discovery.DEFAULTS and
# round_robin.discovery.DEFAULTS — the products must agree
# byte-for-byte on each other's locations.
